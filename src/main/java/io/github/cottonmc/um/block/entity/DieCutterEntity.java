...
public class DieCutterEntity extends BlockEntity {
	...
	public void pulse() {
		...
		if (operation != world.getRecipeManager().getFirstMatch(UMRecipes.DIE_CUTTER, getInventory(), world).orElse(null)) {
			operation = null;
			operationLength = 0;
		}
		if (world.getTime()>=operationStart+operationLength) {
			...
			if (operation==null && !items.get(SLOT_WORK).isEmpty()) {
				operation = world.getRecipeManager().getFirstMatch(UMRecipes.DIE_CUTTER, getInventory(), world).orElse(null);
				if (operation != null) {
